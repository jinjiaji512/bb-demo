...
func GetFullName() string {
	...
	last := lastName[rand.Intn(int(NormalFloat(0, float32(lastNameLen-1)/2, float32(1), float32(lastNameLen-1))))]
	return fmt.Sprintf("%s%s", last, first)
}

func NormalFloat(avg float32, sigma float32, min float32, max float32) float64 {
	//fmt.Println(avg, sigma, min, max)
	v := float64(rand.NormFloat64()*float64(sigma) + float64(avg))
	if v > float64(max) || v < float64(min) {
		return NormalFloat(avg, sigma, min, max)
	}
	return Decimal(v, 2)
}
